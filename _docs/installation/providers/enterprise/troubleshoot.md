...
- To download the script click on **click here** below or run the wget command below.

* [Click here](/installation-scripts/FaveoInstallationScripts/basic-troubleshoot.sh) to download.
...
- Once the file is downloaded to the faveo server we have to provide executable permission to the script, to do the same run the below command inside the directory where the script is present.
```
...
- Once the script is executed it will prompt for faveo root directory which is a necessary value for the script to work.
...
/var/www/html/faveo
```
- Next, Select an Option from the Menu You will be prompted to select one from the following:
 - Example:
...
- If you want to run a single specific check instead of all, select the relevant option by passing optinon number **2 to 12** from the menu when prompted.  [Click here for more details and steps to fix issues](###single-specific-check)

### To Run all checks

- If selected option is **(1) Run all checks** is selected it will prompt and run as below:

 - Example output of option (1) Script Execution Summary
```
...
Enter Faveo root directory path (e.g., /var/www/faveo) /var/www/faveo is the default press enter to use the default value: <<< enter if root directory is differnt from default>>>
Select an option to run:
1) Run all checks
2) SSL Check
3) System Info
...
```
- When it comes to **Port Availablitiy Check** it will prompt the user for custom ports, if any please enter the port number seperated by comma the default ports in the script are **80, 443, 3306, 6379**.
- Once enetered it will continue like below:
```
...
- You will be prompted to select one of the following where you can select options from **2 to 12** which is run the corresponding checks:
...
- It displays inforamtion on System OS, uptime, memory, CPU, disk, and Server resource consumption.
...
#### Service status and verison check:
...
```
- This script output will be logged to **faveo-check.log** inside the same directory where the script is present.
 - To Fix we can try the below command, if the issue persists please reach (**support@faveohelpdesk.com***)
```
systemctl restart <<<service name here>>>
```

#### Faveo Info Check
- This check is used to check faveo releated informations.
...
#### Cron Jobs Check
- This check is used to see cron releated data for faveo releated crons.
...
```
- This script output will be logged to **faveo-check.log** inside the same directory where the script is present.
 - To Fix try the below steps, if the issue persists please reach (**support@faveohelpdesk.com***)
 - If the cron is not there follow **https://docs.faveohelpdesk.com** and select you OS there and follow cron jobs section in the installation steps.
...
#### Billing connection
- This check will check the curl connection between faveo server and billing.faveohelpdesk.com, faveo needs this to validate the license and faveo updates etc..
...
```
- This script output will be logged to **faveo-check.log** inside the same directory where the script is present.
 - To Fix we can try following the below steps, if the issue persists please reach (**support@faveohelpdesk.com***)
 - Try white lisitng this domain in your firewall **billing.faveohelpdesk.com**.


#### Root-Owned Files check
- This check will check if any files are owned by root users inside faveo root directory.
...
- This indicates that list of files and directories are incorrectly owned by root.
...
- This Check is to check whehter the ports are open internally and the services are listenling in the port.

- Enter **11** to check Check if Required Ports are Open for Faveo
...
```
- This script output will be logged to **faveo-check.log** inside the same directory where the script is present.
 - To fix this issue try opening the port in firewall if any is enabled, if the issue persists please reach (**support@faveohelpdesk.com***)

#### Firewall Check

- This is used to check the firewall status from basic firewalls to CSF firewall inside the server.3
...
# Help
- If any queries or updates needed help with please reach **support@faveohelpdesk.com**
