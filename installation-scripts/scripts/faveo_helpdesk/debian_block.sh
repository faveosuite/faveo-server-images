...
##---------- Tested on : Ubuntu 24/22, Debian 11/12/13----------------------------------------##
...
parse_args() {
    ...
    if [[ "$Ssl_option_value" == "paid-ssl" ]]; then
        if [[ -n "$certfile" ]]; then
            _resolved=$(_resolve_cert_arg "$certfile" "/etc/apache2/ssl/faveolocal.crt" "cert")
        ...
        if [[ -n "$keyfile" ]]; then
            _resolved=$(_resolve_cert_arg "$keyfile" "/etc/apache2/ssl/private.key" "key")
        ...
        if [[ -n "$cacertfile" ]]; then
            _resolved=$(_resolve_cert_arg "$cacertfile" "/etc/apache2/ssl/faveorootCA.crt" "cert")
    ...
    if [[ "$MySQL_SEC" == "yes" ]]; then
        if [[ -n "$MySQL_SSLCA" ]]; then
            _resolved=$(_resolve_cert_arg "$MySQL_SSLCA" "/etc/mysql/faveo-mysql-ca.pem" "cert")
        ...
        if [[ -n "$MySQL_SSLCERT" ]]; then
            _resolved=$(_resolve_cert_arg "$MySQL_SSLCERT" "/etc/mysql/faveo-mysql-cert.pem" "cert")
        ...
        if [[ -n "$MySQL_SSLKEY" ]]; then
            _resolved=$(_resolve_cert_arg "$MySQL_SSLKEY" "/etc/mysql/faveo-mysql-key.pem" "key")
 ...
 rollback ()
{
    ...
    [[ "$nats_option" =~ ^(yes|y)$ ]] && {
        ...
        # Remove NATS supervisor worker entry if present
        if [[ -f /etc/supervisor/conf.d/faveo-worker.conf ]]; then
            sed -i \'/\\[program:faveo-Nats\\]/,/^$/d\' /etc/supervisor/conf.d/faveo-worker.conf 2>/dev/null
        fi
        systemctl daemon-reload 2>/dev/null
    ...
    if [[ "$sql_option" =~ ^(mysql|mariadb)$ ]]; then
        ...
        if [[ "$sql_option" == "mariadb" ]]; then
            apt purge -y mariadb-server mariadb-client mariadb-common mariadb-* 2>/dev/null
            rm -f /etc/apt/sources.list.d/mariadb.list
        else
            apt purge -y mysql-server mysql-client mysql-common mysql-* 2>/dev/null
            rm -f /etc/apt/sources.list.d/mysql*.list
            rm -f "$PWD"/mysql-apt-config*.deb
        fi
    ...
    # PHP
    [[ -n "$php_option" ]] && {
        systemctl stop php"$php_option"-fpm 2>/dev/null
        apt purge "php${php_option}*" libapache2-mod-php"$php_option" -y 2>/dev/null
        apt autoremove -y 2>/dev/null
        rm -rf /etc/apt/trusted.gpg.d/sury-keyring.gpg /etc/apt/sources.list.d/*
    ...
    case "$webserver" in
        apache)
            ...
            rm -f /etc/apache2/sites-available/faveo*.conf
            rm -f /etc/apache2/sites-enabled/faveo*.conf
            rm -rf /etc/apache2/ssl
            rm -rf "$certfile" "$keyfile" "$cacertfile"
            apt purge apache2 apache2-utils libapache2-mod-security2 \'libapache2-*\' -y 2>/dev/null
            apt autoremove -y 2>/dev/null
        ...
        nginx)
            ...
            rm -f /etc/nginx/sites-available/faveo*.conf
            rm -f /etc/nginx/sites-enabled/faveo*.conf
            rm -rf /etc/nginx/ssl
            rm -rf "$certfile" "$keyfile" "$cacertfile"
            apt purge nginx nginx-common -y 2>/dev/null
            apt autoremove -y 2>/dev/null
    ...
    case "$ssl_option" in
        certbot)
            ...
            apt purge certbot python3-certbot-apache python3-certbot-nginx -y 2>/dev/null
            ...
            ;;
        self-signed)
            rm -rf /etc/apache2/ssl /etc/nginx/ssl
            rm -f /usr/local/share/ca-certificates/faveorootCA.crt
    ...
    rm -f /etc/supervisor/conf.d/faveo-worker.conf
    apt purge redis-server supervisor -y 2>/dev/null
    apt autoremove -y 2>/dev/null
    echo -e "$red Removed Redis and Supervisor $reset"

    # wkhtmltopdf & IonCube
    apt purge wkhtmltox -y 2>/dev/null
    rm -rf /usr/local/lib/php/ioncube* "$PWD"/ioncube*
    echo -e "$red Removed wkhtmltopdf and IonCube $reset"

    # Faveo cronjob
    (sudo -u www-data crontab -l 2>/dev/null | grep -v "/usr/bin/php /var/www/faveo/artisan schedule:run") | sudo -u www-data crontab -
    echo -e "$red Removed Faveo cronjob $reset"

    # Faveo files & apt sources
    ...
    rm -f /etc/apt/sources.list.d/mariadb* \\
          ...
          /etc/apt/trusted.gpg.d/sury-keyring.gpg \\
          /etc/apt/trusted.gpg.d/mariadb* \\
          /etc/cron.d/faveo-ssl
    update-ca-certificates --fresh 2>/dev/null
    echo -e "$red Removed Faveo files and apt sources $reset"
...
redis() {
    ...
    apt update && apt install -y redis-server supervisor || {
        echo -e "$red Failed to install Redis or Supervisor. Rolling back... $reset"
        rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"
        return 1
    }

    systemctl enable --now redis-server supervisor || {
...
EOF
    ...
    # Add cron only if it doesn't exist
    (sudo -u www-data crontab -l 2>/dev/null | grep -q "artisan schedule:run") || \
        (sudo -u www-data crontab -l 2>/dev/null; echo "#* * * * * /usr/bin/php /var/www/faveo/artisan schedule:run 2>&1") | \
        sudo -u www-data crontab -
...
meilisearch () {
    ...
    curl -L https://install.meilisearch.com -o meilisearch || {
...
EOF
    ...
    systemctl restart php"${Php_option_value}"-fpm
...
extensions () {
    ...
    # Add zend_extension to PHP configs
    ini_types=("cli" "fpm")
    ...
    # Add zend_extension to PHP configs
    for ini in "${ini_types[@]}"; do
        ini_file="/etc/php/${Php_option_value}/${ini}/php.ini"

        if [[ -f "$ini_file" ]]; then
            sed -i "2 a zend_extension = \"$php_path/ioncube_loader_lin_${Php_option_value}.so\"" "$ini_file" || {
                echo -e "$red Failed to update php.ini for $ini. Rolling back... $reset"
                rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"
                return 1
            }
        else
            echo -e "$yellow Skipping $ini_file (not found) $reset"
        fi
    done
    ...
    case $derivative in
        2004|11)
            apt install -y wkhtmltopdf || {
                echo -e "$red Failed to install wkhtmltopdf. Rolling back... $reset";
                rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile";
                return 1;
            }
            ;;
        2204|2404)
            apt install -y libfontenc1 xfonts-75dpi xfonts-base xfonts-encodings xfonts-utils || {
                echo -e "$red Failed to install font packages. Rolling back... $reset"
                rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"
                return 1
            }
            wkurl="https://github.com/wkhtmltopdf/packaging/releases/download/0.12.6.1-3/wkhtmltox_0.12.6.1-3.jammy_amd64.deb"
            ;;
        12)
            wkurl="https://github.com/wkhtmltopdf/packaging/releases/download/0.12.6.1-3/wkhtmltox_0.12.6.1-3.bookworm_amd64.deb"
            ;;
        13)
            wkurl="https://github.com/wkhtmltopdf/packaging/releases/download/0.12.6.1-3/wkhtmltox_0.12.6.1-3.bookworm_amd64.deb"
            ;;
        *)
            echo -e "$red Unsupported OS for wkhtmltopdf. Rolling back... $reset"
            rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"
            return 1
            ;;
    esac

    # Install wkhtmltopdf if URL is defined
    if [[ -n "$wkurl" ]]; then
        wget -q "$wkurl" || { echo -e "$red Failed to download wkhtmltopdf. Rolling back... $reset"; rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"; return 1; }
        dpkg -i ./wkhtmltox_*.deb
        apt --fix-broken install -y || { echo -e "$red Failed to fix broken dependencies. Rolling back... $reset"; rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"; return 1; }
        rm -f ./wkhtmltox_*.deb
    fi

    # Restart PHP + Webserver
    systemctl restart php"${Php_option_value}"-fpm || { echo -e "$red Failed to restart PHP-FPM. Rolling back... $reset"; rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"; return 1; }

    if [[ "$Webserver_option_value" == "apache" ]]; then
        systemctl restart apache2 || { echo -e "$red Failed to restart Apache. Rolling back... $reset"; rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"; return 1; }
    elif [[ "$Webserver_option_value" == "nginx" ]]; then
...
# Attempts to set up and install MariaDB (10.6 or 11.8 only).
...
attempt_mariadb() {
    ...
    echo -e "$cyan Available MariaDB server versions in configured repo: $(apt-cache madison mariadb-server 2>/dev/null | awk '{print $3}' | tr '\n' ' ')$reset"
    ...
    SQL_Version_Installed=$(dpkg-query -W -f='${Version}' mariadb-server 2>/dev/null)

    # Final safety check: confirm what actually got installed is one of the
    # two allowed series.
    SQL_Version_NoEpoch="${SQL_Version_Installed#*:}"
    if [[ ! "$SQL_Version_NoEpoch" =~ ^10\.6\. ]] && [[ ! "$SQL_Version_NoEpoch" =~ ^11\.8\. ]]; then
...
# Attempts to set up and install MySQL (8.0 or 8.4 only).
# Returns 0 on success (sets SQL_Version_Installed and Sql_option_value=mysql),
# 1 on failure. Does NOT rollback - caller decides what to do next.
attempt_mysql() {
    echo -e "$yellow Setting up MySQL repository... $reset"

    CODENAME=$(lsb_release -sc)
    declare -A MySQL_Channel_Map=(["8.0"]="mysql-8.0" ["8.4"]="mysql-8.4-lts")
    MySQL_Allowed_Versions=("8.0" "8.4")
    MySQL_Channel=""
    MySQL_Version=""

    for v in "${MySQL_Allowed_Versions[@]}"; do
        channel="${MySQL_Channel_Map[$v]}"
        cat <<EOF > /etc/apt/sources.list.d/mysql.list
deb [arch=amd64 trusted=yes] http://repo.mysql.com/apt/${OS_ID} ${CODENAME} ${channel}
deb [arch=amd64 trusted=yes] http://repo.mysql.com/apt/${OS_ID} ${CODENAME} mysql-tools
EOF
        apt-get update -qq 2>/dev/null

        if apt-cache madison mysql-server 2>/dev/null | grep -q .; then
            MySQL_Channel="$channel"
            MySQL_Version="$v"
            echo -e "$green MySQL $v (channel '$channel') is available for $OS_ID $CODENAME. $reset"
            echo -e "$cyan Available MySQL server versions in this channel: $(apt-cache madison mysql-server 2>/dev/null | awk '{print $3}' | tr '\n' ' ')$reset"
            break
        else
            echo -e "$yellow MySQL $v (channel '$channel') is not available for $OS_ID $CODENAME. $reset"
        fi
    done

    if [[ -z "$MySQL_Channel" ]]; then
        echo -e "$yellow Neither supported MySQL version (8.0, 8.4) is available for $OS_ID $CODENAME. $reset"
        rm -f /etc/apt/sources.list.d/mysql.list
        return 1
    fi
    ...
    apt-get install -y mysql-server || {
        echo -e "$yellow MySQL installation from the official repository failed. $reset"
        return 1
    }

    systemctl enable mysql && systemctl start mysql

    SQL_Version_Installed=$(dpkg-query -W -f='${Version}' mysql-server 2>/dev/null)

    # Final safety check: confirm what actually got installed is one of the
    SQL_Version_NoEpoch="${SQL_Version_Installed#*:}"
    if [[ ! "$SQL_Version_NoEpoch" =~ ^8\.0\. ]] && [[ ! "$SQL_Version_NoEpoch" =~ ^8\.4\. ]]; then
...
sql_installation() {
    ...
    # Update system & install dependencies (mysql-client covers both engines
    # for the remote-connection case below)
    apt-get update && apt-get install -y lsb-release ca-certificates apt-transport-https software-properties-common gnupg2 wget zip unzip curl nano default-mysql-client
    ...
    if [[ "$Remote_Sql_option_value" == "yes" ]]; then
        ...
        elif [[ "$Preferred_Sql_option_value" == "mysql" ]]; then
            if ! attempt_mysql; then
...
certbot_apache() {
    ...
    apt-get install -y python3-certbot-apache certbot
    ...
    echo "45 2 * * 6 /usr/bin/certbot renew --quiet && /bin/systemctl restart apache2.service" | sudo tee /etc/cron.d/faveo-ssl
    sleep 1
...
self_signed_apache() {
    ...
    openssl x509 -req -sha256 -days 7300 -in "$SSL_DIR/faveoroot.csr" -signkey "$SSL_DIR/faveoroot.key" -out "$SSL_DIR/faveorootCA.crt"
    ...
    openssl x509 -req -in "$SSL_DIR/faveolocal.csr" -CA "$SSL_DIR/faveorootCA.crt" -CAkey "$SSL_DIR/faveoroot.key" \
        -CAcreateserial -out "$SSL_DIR/faveolocal.crt" -days 7300 -sha256
    ...
    cp "$SSL_DIR/faveorootCA.crt" /usr/local/share/ca-certificates/
    update-ca-certificates --fresh

    # Apache SSL site configuration
    apt install -y libapache2-mod-security2
    a2enmod security2 rewrite headers http2 ssl
    a2dismod php"$Php_option_value" mpm_prefork
    a2enmod mpm_event proxy_fcgi setenvif
    a2enconf php"$Php_option_value"-fpm
...
EOF
        a2enmod proxy proxy_wstunnel headers proxy_http
    else
cat <<EOF > /etc/apache2/sites-available/faveo-ssl.conf
<IfModule mod_ssl.c>
  FileETag None
...
EOF
    ...
    systemctl restart php"$Php_option_value"-fpm apache2
    # Test HTTPS
    local test=$(curl -ks https://"$DomainName"/test.html)
    if [[ "$test" == "Test" ]]; then
        echo -e "$green Self Siged SSL successfully configured for $DomainName. $reset"
        sleep 1
        sql_installation "$DomainName" "$Email" "$LicenseCode" "$OrderNumber" "$ReleaseSelection" "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value"
    else
...
# Self Signed Nginx instrallation:
self_signed_nginx() {
    echo -e "$yellow Generating Self Signed SSL certificates for $DomainName... $reset"
    ...
    # Nginx site config
    if [[ "$Nats_option_value" =~ ^(y|yes)$ ]]; then
cat <<EOF > /etc/nginx/sites-available/faveo-ssl.conf
server {
    listen 80;
    listen [::]:80;
    ...
    location ~ \.php\$ {
        ...
        fastcgi_pass unix:/var/run/php/php$Php_option_value-fpm.sock;
...
EOF
    ...
    # Enable site safely
    if [[ ! -L /etc/nginx/sites-enabled/faveo-ssl.conf ]]; then
        ln -s /etc/nginx/sites-available/faveo-ssl.conf /etc/nginx/sites-enabled/
    fi

    systemctl restart php"$Php_option_value"-fpm nginx
...
EOF
    ...
    cp $cacertfile /usr/local/share/ca-certificates/
    update-ca-certificates --fresh
    # Enable site and reload services
    a2ensite faveo-ssl.conf
    systemctl restart php"$Php_option_value"-fpm apache2
...
EOF
    ...
    ln -sf /etc/nginx/sites-available/faveo-ssl.conf /etc/nginx/sites-enabled/
...
web_server_configuration() {
    ...
    verify_web_server() {
        local test_result=$(curl -s "http://$DomainName/test.html")
    ...
    if [[ "$Webserver_option_value" == "apache" ]]; then
        ...
        if [[ "$os_id" == "ubuntu" ]]; then
            ...
            echo "deb [signed-by=/usr/share/keyrings/deb.sury.org-apache2.gpg] https://packages.sury.org/apache2/ $(lsb_release -sc) main" > /etc/apt/sources.list.d/apache2.list
            apt-get update && apt-get install apache2 -y
        fi

        systemctl enable apache2
        mkdir -p "$FAVEO_WEBROOT"
        a2enmod proxy proxy_wstunnel headers proxy_http rewrite
        if [[ "$Nats_option_value" =~ ^(y|yes)$ ]]; then
...
EOF
        ...
        ln -sf /etc/nginx/sites-available/faveo.conf /etc/nginx/sites-enabled/
        systemctl restart nginx
        verify_web_server
...
dependencies() {
    ...
    # Detect OS
    OS=$(grep -qs "ubuntu" /etc/os-release && echo "ubuntu" || echo "debian")
    ...
    PHP_PACKAGES=(php"$Php_option_value" php"$Php_option_value"-mysql php"$Php_option_value"-cli php"$Php_option_value"-common \
        php"$Php_option_value"-fpm php"$Php_option_value"-soap php"$Php_option_value"-gd php"$Php_option_value"-opcache \
        php"$Php_option_value"-mbstring php"$Php_option_value"-zip php"$Php_option_value"-bcmath php"$Php_option_value"-intl \
        php"$Php_option_value"-xml php"$Php_option_value"-curl php"$Php_option_value"-imap php"$Php_option_value"-ldap \
        php"$Php_option_value"-gmp php"$Php_option_value"-redis php"$Php_option_value"-memcached)

    apt install -y "${PHP_PACKAGES[@]}" || { echo -e "$red PHP installation failed. $reset"; rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"; return; }

    # Apache module if needed
    if [[ $Webserver_option_value == "apache" ]]; then
        apt install -y libapache2-mod-php"$Php_option_value"
    fi

    # Configure PHP settings
    PHP_INI_PATHS=(/etc/php/"$Php_option_value"/fpm/php.ini /etc/php/"$Php_option_value"/cli/php.ini)
    [[ $Webserver_option_value == "apache" ]] && PHP_INI_PATHS+=("/etc/php/$Php_option_value/apache2/php.ini")
...
# Nats Installation:
nats_install() {
    local nats_choice="${Nats_option_value,,}"  # normalize to lowercase
...
node_installation() {
    ...
    if [[ "$node_choice" =~ ^(y|yes)$ ]]; then
        ...
        apt update && apt install -y git curl wget zip unzip

        # Install Node.js 22.x
        curl -fsSL https://deb.nodesource.com/setup_22.x | bash - || { echo -e "$red Failed to setup Node.js repo.$reset"; rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"; exit 1; }
        apt update && apt install -y nodejs || { echo -e "$red Failed to install Node.js.$reset"; rollback "$Sql_option_value" "$Php_option_value" "$Meili_option_value" "$Webserver_option_value" "$Node_option_value" "$Nats_option_value" "$Ssl_option_value" "$certfile" "$keyfile" "$cacertfile"; exit 1; }

        # Puppeteer global install
    ...
    while true; do
        ...
        case "${ssl_choice,,}" in
            ...
            3)
                ...
                ssl_files() {
                    local DIR_NAME="/etc/apache2/ssl"
                    mkdir -p "$DIR_NAME"
...
sql_server_selection() {
    ...
    if [[ "$MySQL_SEC" == "yes" ]]; then
        ...
        _collect_mysql_ssl_material() {
            ...
            local dest_file="$2"      # e.g. /etc/mysql/faveo-mysql-ca.pem
        ...
        _collect_mysql_ssl_material "MySQL SSL CA certificate (required)" "/etc/mysql/faveo-mysql-ca.pem" "MySQL_SSLCA" "cert" "yes"
        _collect_mysql_ssl_material "MySQL SSL client certificate (optional)" "/etc/mysql/faveo-mysql-cert.pem" "MySQL_SSLCERT" "cert" "no"
        _collect_mysql_ssl_material "MySQL SSL client key (optional)" "/etc/mysql/faveo-mysql-key.pem" "MySQL_SSLKEY" "key" "no"
...
php_selection() {
    ...
    while true; do
        ...
        echo -e "$cyan You may choose your preferred PHP version (e.g., 8.2, 8.4, x.x). Default is 8.4.$reset"
        read -p "$(echo -e "$yellow Enter PHP version [default: 8.4]: $reset")" Php_option_choice

        # Apply default if user pressed Enter with no input
        ...
        if [[ "$Php_option_choice" =~ ^([8-9]|[1-9][0-9])\.[0-9]+$ ]]; then
            ...
            retries=$((retries + 1))
            echo -e "$red Invalid PHP version. Please enter a version 8.4 or above. ($retries/$max_retries)$reset"
            if [ $retries -ge $max_retries ]; then
                echo -e "$red Too many invalid attempts. Please re-run the script.$reset"
                exit 1
            fi
...
attributes () {
    ...
    # Faveo environment  (default: prod)
    if [[ -z "$ENV_SETUP" ]]; then
        faveo_env_selection
    fi
    ...
    echo -e "${cyan}${bold}       Faveo Installation Summary                      ${reset}"
    echo -e "${cyan}${bold}╚══════════════════════════════════════════════════════╝${reset}"
    ...
    echo -e "${cyan}${bold}╚══════════════════════════════════════════════════════╝${reset}"
    read -p "$(echo -e "Continue ($green y $reset/$red n $reset)? ")" REPLY
    if [[ ! $REPLY =~ ^([yY][eE][sS]|[yY])$ ]]; then
        echo -e "$red Cancelled. Please rerun the script. $reset"
        exit 1
    fi
